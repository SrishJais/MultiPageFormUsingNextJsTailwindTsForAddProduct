...
import { Combination, CombinationObj, Variant } from "@/typescriptTypes/types";
...
const Combinations = () => {
  ...
  const [productCombinationsData, setProductCombinationsData] = useState<
  ...
  const [errors, setErrors] = useState<
    Record<string, { sku: string; quantity: string }>
  >({});
  ...
  useEffect(() => {
    function generateCombinations(
      ...
      function helper(index: number, currentCombo: string[], key: string) {
        ...
        for (const value of variants[index].values) {
          helper(index + 1, [...currentCombo, value], key + value[0]);
        }
      }
    ...
    if (tempProductObj?.variants) {
      setProductCombinationsData(generateCombinations(tempProductObj.variants));
    }
  }, [tempProductObj]); // Added dependency

  // Validation function
  const isValidateReqFields = () => {
    const tempErrors: Record<string, { sku: string; quantity: string }> = {};
  ...
  const handleQuantityChange = (key: keyof CombinationObj, value: string) => {
    setProductCombinationsData((prev) => ({
      ...prev,
      [key]: {
        ...prev[key], // Preserve existing fields
        quantity: value.trim() === "" ? null : Number(value.trim()), // Convert string to number
      },
    }));
